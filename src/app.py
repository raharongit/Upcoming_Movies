...
def poster(genre, page_no):
    ...
    soup = BeautifulSoup(content, "html.parser")
    soup2 = soup.find('div', {'class':'movie-ls-group'})
    elements = soup2.find_all('img', {'class': 'visual-thumb'})
...
genres= ['Action & Adventure', 'Drama', 'Comedy', 'Kids', 'Horror',
        'Romance', 'Sci-Fi', 'Animated', 'Documentaries',
        '3D', 'Suspense', 'Indie', 'Foreign',
        'Special Events', 'Western', 'War',
        'Film, TV & Radio', 'Music/Performing Arts',
        ]
...
def home(genre = 'Drama'):
    ...
    return render_template('movieslist.html',
                           ...
                           youtube_link='https://www.youtube.com/search?q=',
                           elements=new_title,
